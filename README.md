...
一個互動式的赤壁之戰 3D 戰場：十一幕時間軸從「大軍南下」演到「天下三分」，
可以像歷史節目一樣自動播映（電影運鏡 + letterbox），也可以像遊戲一樣
隨時拖曳滑鼠接管鏡頭、自由探索戰場。

- **地形**：程序化生成的長江戰場——赤壁紅崖、烏林、夏口、漢水支流、華容道沼澤、江陵與襄陽城
- **三方兵力**：曹操藍、孫權紅、劉備綠；軍旗以「曹／孫／劉」大字呈現，遠看即可識別
- **時間軸十一幕**：大軍南下 → 孫劉同盟 → 初戰赤壁 → 群英會・蔣幹盜書 → 草船借箭 → 鐵索連環 → 苦肉計 → 東風驟起 → 火燒赤壁 → 敗走華容 → 天下三分
- **每一幕呈現**：陣型與移動方向（動態箭頭）、將領姓名、重大事件標注、三方戰力消長、艦隊狀態（正常／連環／著火／殘骸）
- **計策發動特效**：反間計、草船借箭、連環計、苦肉計、借東風、火攻——以全螢幕書法計策卡呈現，並區分「史」（正史）與「演義」（三國演義）
- **天氣與戰場特效**：東南風粒子流、草船借箭大霧鎖江、華容道大雨、GPU 火焰與濃煙、火光映江、箭雨與火矢（漢代無火炮，遠程武器以弓弩火矢呈現）、鏡頭震動
- **場景地標**：南屏山七星壇（借東風時光柱衝天）、烏林曹營、赤壁吳營、四座城池
- **現代怎麼看**：各幕附知識框——軍中大疫的血吸蟲假說、東南風的氣象學解釋、草船借箭的史實原型（孫權濡須受箭）、詐降書為何是勝負手
- **互動**：點擊任一部隊查看兵力與背景；時間軸可點擊跳轉任一幕；←／→ 切幕、空白鍵播放暫停；速度 0.5× / 1× / 2×；「？ 說明」內附史料來源
- **品質細節**：配樂開關與樂風記憶（localStorage）、行動裝置自動降載（粒子數／解析度）、持續掉幀自動降解析度、手機直立提示
...
按下「自動播映」或「自由探索」後配樂自動開始（預設開啟）。全程式化生成（Web Audio API），**零音檔、維持單一 HTML**。

- **開關**：控制列「♪ 配樂」按鈕，或按 `M`
- **切換樂風**：控制列「樂風」按鈕，或按 `S`，三種風格循環：
  - **戰記交響**——光榮歷史大河風：弦樂低鳴、大太鼓、箏分解和弦、尺八主題、銅管揚起（D 小調五聲）
  - **雅樂幽玄**——笙的和音雲、尺八長吟、疏落箏音、銅鑼餘韻（F 都節音階）
  - **太鼓戰陣**——重太鼓陣型、締太鼓、法螺貝號角、持續低音（適合火攻段落）
- **隨幕變化**：十一幕各有配樂強度（太鼓密度、音量、微速隨之變化）——「火燒赤壁」最烈，「天下三分」收束

**音色為真實樂器取樣**（FluidR3 SoundFont：尺八、箏、三味線、太鼓、弦樂合奏、銅管、法國號、管鐘、簧風琴作笙），頁面載入時自 CDN（jsDelivr，備援 GitHub Pages）預載約 25 個單音樣本（~數百 KB），播放時以最近樣本變速到目標音高。**若 CDN 不可用則逐聲部自動退回程序化合成音色**（太鼓＝正弦掃頻＋皮膜噪聲、尺八＝正弦＋氣息噪聲＋顫音等），離線亦可運作。殘響為程序化生成的卷積殘響，全程不需任何本地音檔。
...
1. **設計**：模型先決定美術方向（深墨金箔的歷史紀錄片質感、書法計策卡、letterbox 電影黑邊），再規劃 3D 架構：程序化地形、十一幕時間軸資料驅動引擎、每幕運鏡腳本（line / orbit / follow 三種鏡頭）。
...
- 十一幕時間軸：大軍南下、孫劉同盟、初戰赤壁、群英會、草船借箭、鐵索連環、苦肉計、
  借東風、火燒赤壁、敗走華容、天下三分
